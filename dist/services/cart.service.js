...
exports.mergeGuestCartIntoUserCart = exports.removeCartItem = exports.recalcRedisCart = exports.updateCartItem = exports.addToCart = exports.getOrCreateCart = void 0;
const prisma_1 = require("../lib/prisma");
const redis_1 = __importDefault(require("../utils/redis"));
const CART_EXPIRATION_SECONDS = 60 * 60 * 24 * 7;
const getOrCreateCart = async (params) => {
    ...
    if (!userId) {
        ...
        const cachedGuestCart = await redis_1.default.get(guestKey);
        if (cachedGuestCart) {
            try {
                return JSON.parse(cachedGuestCart);
            }
            catch (e) {
    ...
    const cachedUserCart = await redis_1.default.get(userKey);
    if (cachedUserCart) {
        try {
            return JSON.parse(cachedUserCart);
        }
        catch (e) {
    ...
    const dbCart = await prisma_1.prisma.cart.upsert({
        where: { userId: userId },
        update: {},
    ...
    await redis_1.default.set(userKey, JSON.stringify(normalizedCart), {
        EX: 60 * 60
    });
...
productId, quantity, sessionId, userId) => {
    ...
    if (!userId && sessionId) {
        ...
        const guestKey = `cart:guest:${sessionId}`;
        const cachedCart = await redis_1.default.get(guestKey);
        ...
        await redis_1.default.set(guestKey, JSON.stringify(cart), {
    ...
    // ==========================================
    if (userId) {
        const cartItem = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        await redis_1.default.del(`cart:user:${userId}`);
...
const updateCartItem = async (productId, quantity, sessionId, userId) => {
    // ==========================================
    // PATH A: GUEST USER (REDIS ONLY)
    // ==========================================
    if (!userId && sessionId) {
        ...
        cart = (0, exports.recalcRedisCart)(cart);
        await redis_1.default.set(guestKey, JSON.stringify(cart), {
            EX: CART_EXPIRATION_SECONDS,
        });
...
const recalcRedisCart = (cart) => {
    cart.subtotal = cart.items.reduce((sum, item) => sum + item.lineTotal, 0);
    cart.total = cart.subtotal; // Add tax or shipping logic here later if needed
    return cart;
};
exports.recalcRedisCart = recalcRedisCart;
const removeCartItem = async (
// cartId: string | null,
productId, sessionId, userId // <-- ADDED: Required for Auth cache invalidation
) => {
    // ==========================================
    // PATH A: GUEST USER (REDIS ONLY)
    // ==========================================
...
const mergeGuestCartIntoUserCart = async (sessionId, userId) => {
    ...
    const cachedCart = await redis_1.default.get(redisKey);
    // 1. Exit early if there's nothing to merge
    if (!cachedCart)
        return;
    const guestCart = JSON.parse(cachedCart);
